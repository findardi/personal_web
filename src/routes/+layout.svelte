<script lang="ts">
	import '../app.css';
	import AOS from 'aos';
	import icon from '$lib/assets/icon2.png';
...
<svelte:head>
	...
	<title>{page.data.title ? `${page.data.title} | Ardi Sasongko` : `Hei, Come Here!`}</title>
	<meta
		name="description"
		content={page.data.description ? `${page.data.description}` : defaultDesc}
	/>
</svelte:head>
