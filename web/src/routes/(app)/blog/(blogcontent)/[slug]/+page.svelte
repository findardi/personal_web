<script lang="ts">
	...
	let openModal: (() => void) | undefined = $state();
</script>
		...
		<div class="mt-6 flex flex-col space-y-4 p-0">
			<div class="flex w-full items-center justify-between">
				<div class="flex items-center justify-start gap-2">
					<p class="text-sm font-bold">{blog.createdBy}</p>
					<time datetime="2025-09-19" class="text-xs font-bold text-base-content/60"
						>{blog.createdAt}</time
					>
				</div>
				<div class={data.isSuper ? 'flex space-x-1' : 'hidden'}>
					<button
						class="btn rounded-md btn-sm btn-primary md:btn-md"
						onclick={() => goto(`/blog/${blog.slug}/edit`)}>Edit</button
					>
					<button class="btn rounded-md btn-sm btn-error md:btn-md" onclick={() => openModal?.()}
						>Delete</button
					>
				</div>
			</div>
			<Modal bind:open={openModal} name={'blog-modal'} />
